...
def _find_by_name(client: SnipeITClient, name: str, cache: Dict[str, Optional[dict]]) -> Optional[dict]:
    ...
    rows = client._paginate_rows("/api/v1/suppliers", search=name)
    for row in rows:
    ...
    for index, row in enumerate(rows, start=2):
        try:
            ...
            payload = {
                ...
                "contact": _str(row.get("contact")),
                "phone": _str(row.get("phone")),
                "fax": _str(row.get("fax")),
            }
            payload = {k: v for k, v in payload.items() if v not in ("", None)}

            client._request("POST", "/api/v1/suppliers", body=payload)
            summary["created"] += 1
            ...
            print(f"Suppliers sync failed at row {index}: {exc}")
