const LOCATIONS_API_ENDPOINT = (APILib.getEndpointLocations && APILib.getEndpointLocations()) || PropertiesService.getScriptProperties().getProperty("API_Endpoint_locations");
const LOCATIONS_API_KEY = (APILib.getApiKey && APILib.getApiKey()) || (APILib.getMyApiKey && APILib.getMyApiKey()) || "";
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu("Snipe-IT Locations")
    .addItem("Sync this sheet to Snipe-IT Locations", "syncLocationsCurrentSheet")
    .addToUi();
}

function syncLocationsCurrentSheet() {
  syncSimpleEntity_("Locations", LOCATION_HEADERS, LOCATIONS_API_ENDPOINT, LOCATIONS_API_KEY, {
    createdLabel: "Created",
    skippedLabel: "Skipped",
  });
}

function syncSimpleEntity_(entityName, headers, endpoint, apiKey, labels) {
  if (!endpoint) {
    SpreadsheetApp.getUi().alert(`Missing endpoint for ${entityName}.`);
    return;
  }
  if (!apiKey) {
  ...
  try {
    ...
    if (code === 200) {
      ...
      SpreadsheetApp.getUi().alert(
        `${entityName} sync finished!\n` +
        `${labels.createdLabel}: ${body.created || 0}\n` +
        `${labels.skippedLabel}: ${body.skipped || 0}\n` +
