const SUPPLIERS_API_ENDPOINT = (APILib.getEndpointSuppliers && APILib.getEndpointSuppliers()) || PropertiesService.getScriptProperties().getProperty("API_Endpoint_suppliers");
const SUPPLIERS_API_KEY = (APILib.getApiKey && APILib.getApiKey()) || (APILib.getMyApiKey && APILib.getMyApiKey()) || "";
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu("Snipe-IT Suppliers")
    .addItem("Sync this sheet to Snipe-IT Suppliers", "syncSuppliersCurrentSheet")
    .addToUi();
}

function syncSuppliersCurrentSheet() {
  if (!SUPPLIERS_API_ENDPOINT) {
    SpreadsheetApp.getUi().alert("Missing endpoint for Suppliers.");
    return;
  }
  if (!SUPPLIERS_API_KEY) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const rowObj = {};
    SUPPLIER_HEADERS.forEach(header => {
  ...
  const options = {
    ...
    headers: { "x-api-key": SUPPLIERS_API_KEY },
    payload,
    muteHttpExceptions: true,
  };

  try {
    const response = UrlFetchApp.fetch(SUPPLIERS_API_ENDPOINT, options);
